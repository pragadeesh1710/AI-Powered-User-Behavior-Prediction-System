...
def courses():
    ...
    return render_template("products.html", courses=all_courses, categories=categories,
                           q=q, selected_category=category or "All")
...
def course_details(course_id):
    ...
    return render_template("course_details.html", course=course, related=related,
                           favorite=favorite, user_rating=user_rating)
...
def dashboard():
    ...
    counts = {}
    for activity_type, key in [("view", "viewed"), ("enroll", "enrolled"), ("complete", "completed"), ("favorite", "favorites_count")]:
        cursor.execute(
    ...
    return render_template("dashboard.html", **counts, recent=recent, learning=learning,
                           favorites=favorites, recommendations=recommendations, interests=interests)
...
def recommendations():
    ...
    interests = get_user_interest_summary(user_id)
    return render_template("recommendations.html", recommendations=recs, interests=interests)
...
def admin():
    ...
    cursor.execute("""
        SELECT ua.activity_id, u.name AS user_name, c.name AS course_name,
               ua.activity_type, ua.timestamp
        FROM user_activity ua
    ...
    return render_template("admin.html", courses=all_courses, activity_logs=activity_logs,
